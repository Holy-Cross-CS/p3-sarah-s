# Collaboration Log:
#      9/23             Talked to Joe Townsend about running the webserver and  how to download web root.
#      10/11            Talked to Emily Bubonovich on what output is to be expected from whisper implementation.
#!/usr/bin/env python3
...
# Updated: 23 September 2024 - Sarah Stokes for project 2
# Updated: 8  October   2024 - Sarah Stokes for project 3 (adding elements for whisper protocol)
#                            - Added code from lines 56 - 65  
...
topics = {
    "whatever": {"messages": [], "likes": 0, "version": 0},
    "holycross": {"messages": [], "likes": 0, "version": 0},
    "music": {"messages": [], "likes": 0, "version": 0}
}
topics_version_num = 0
...
# Made Interactive by entering a name and a favorite color.
def handle_http_get_hello(req, conn):
    cookies = {cookie.split('=')[0]: cookie.split('=')[1] for cookie in (get_cookies(req.headers) or "").split("; ") if '=' in cookie}
    query_params = urllib.parse.parse_qs(urllib.parse.urlparse(req.path).query)
    name = query_params.get("name", [cookies.get("name", "World")])[0]
    favcolor = query_params.get("favcolor", [cookies.get("favcolor", "blue")])[0]
    ...
    <body>
        ...
        <form method="get">
            <label>Enter your name: <input type="text" name="name" value="{name}"></label><br>
            <label>Favorite color: <input type="text" name="favcolor" value="{favcolor}"></label><br>
            <input type="submit" value="Update">
        </form>
    </body>
    </html>
    """
    resp = Response("200 OK", "text/html", html_content.encode())
...
def handle_http_get_whoami(req, conn):
    ...
    user_agent = get_header_value(req.headers, "User-Agent") or "Unknown browser"
    cookies = get_cookies(req.headers) or "No cookies found"
    ...
    """
    return Response("200 OK", "text/html", html_content.encode())
...
def handle_http_get(req, conn):
    ...
    if req.path == "/status":
        ...
        return handle_http_get_hello(req, conn)
...
def handle_get_topics(connection, version):
    with topics_lock:
...
def handle_post_message(connection, body):
    # Parse the body to extract topics and the message
    # This is a simplification, you'll need to parse the actual message format
    topics_list, message = parse_body(body)

    with topics_lock:
        for topic in topics_list:
            if topic not in topics:
                topics[topic] = {"messages": [], "likes": 0, "version": 0}
            topics[topic]["messages"].append(message)
            topics[topic]["version"] += 1
